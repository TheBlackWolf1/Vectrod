...
_PUNCT_MAP = {
    ...
    '!': lambda L,R,W,CX,S,T,sf,fam,adv: [vbar(CX,CAP,BASE-S*3,S,'stem'),
                                            oval(CX,BASE-S,S,S,'terminal')],
    '?': lambda L,R,W,CX,S,T,sf,fam,adv: [arc(CX,CAP+W//4,W//3,W//4,200,-20,S,'arc'),
                                            vbar(CX,(CAP+BASE)//2,BASE-S*3,S,'stem'),
                                            oval(CX,BASE-S,S,S,'terminal')],
    ':': lambda L,R,W,CX,S,T,sf,fam,adv: [oval(CX,(CAP+BASE)//2-S*2,S,S,'terminal'),
                                            oval(CX,BASE-S,S,S,'terminal')],
    ';': lambda L,R,W,CX,S,T,sf,fam,adv: [oval(CX,(CAP+BASE)//2-S*2,S,S,'terminal'),
                                            oval(CX,BASE-S,S,S,'terminal'),
                                            diag(CX-S//2,BASE,CX,BASE+S*2,S//2,'terminal')],
    '-': lambda L,R,W,CX,S,T,sf,fam,adv: [hbar(L+W//4,R-W//4,(CAP+BASE)//2,S,'bar')],
    '_': lambda L,R,W,CX,S,T,sf,fam,adv: [hbar(L,R,BASE+S//2,S,'bar')],
    '/': lambda L,R,W,CX,S,T,sf,fam,adv: [diag(R-S,CAP,L,BASE,S,'stem')],
    '\\': lambda L,R,W,CX,S,T,sf,fam,adv: [diag(L,CAP,R-S,BASE,S,'stem')],
    '|': lambda L,R,W,CX,S,T,sf,fam,adv: [vbar(CX,CAP,BASE,S,'stem')],
    # Brackets
    '(': lambda L,R,W,CX,S,T,sf,fam,adv: [arc(CX+W//4,(CAP+BASE)//2,W//3,(BASE-CAP)//2,120,240,S,'arc')],
    ')': lambda L,R,W,CX,S,T,sf,fam,adv: [arc(CX-W//4,(CAP+BASE)//2,W//3,(BASE-CAP)//2,300,60,S,'arc')],
    '[': lambda L,R,W,CX,S,T,sf,fam,adv: [vbar(L+S,CAP,BASE,S,'stem'),
                                            hbar(L+S,CX,CAP,S//2,'bar'),
                                            hbar(L+S,CX,BASE,S//2,'bar')],
    ']': lambda L,R,W,CX,S,T,sf,fam,adv: [vbar(R-S,CAP,BASE,S,'stem'),
                                            hbar(CX,R-S,CAP,S//2,'bar'),
                                            hbar(CX,R-S,BASE,S//2,'bar')],
    '{': lambda L,R,W,CX,S,T,sf,fam,adv: [arc(CX,(CAP+BASE)//4,S*2,S*2,180,0,S//2,'arc'),
                                            vbar(CX-S,(CAP+BASE)//4,(CAP+BASE)*3//4,S//2,'stem'),
                                            arc(CX,(CAP+BASE)*3//4,S*2,S*2,0,180,S//2,'arc')],
    '}': lambda L,R,W,CX,S,T,sf,fam,adv: [arc(CX,(CAP+BASE)//4,S*2,S*2,0,180,S//2,'arc'),
                                            vbar(CX+S,(CAP+BASE)//4,(CAP+BASE)*3//4,S//2,'stem'),
                                            arc(CX,(CAP+BASE)*3//4,S*2,S*2,180,0,S//2,'arc')],
    # Math & symbols
    '+': lambda L,R,W,CX,S,T,sf,fam,adv: [hbar(L+S,R-S,(CAP+BASE)//2,S,'bar'),
                                            vbar(CX,CAP+W//4,BASE-W//4,S,'stem')],
    '=': lambda L,R,W,CX,S,T,sf,fam,adv: [hbar(L+S,R-S,(CAP+BASE)//2-S*2,S,'bar'),
                                            hbar(L+S,R-S,(CAP+BASE)//2+S*2,S,'bar')],
    '*': lambda L,R,W,CX,S,T,sf,fam,adv: [hbar(L+S,R-S,(CAP+BASE)//2,S//2,'bar'),
                                            diag(L+S,CAP+W//3,R-S,BASE-W//3,S//2,'stem'),
                                            diag(R-S,CAP+W//3,L+S,BASE-W//3,S//2,'stem')],
    '@': lambda L,R,W,CX,S,T,sf,fam,adv: [oval(CX,(CAP+BASE)//2,W//2-S,(BASE-CAP)//2-S,'bowl'),
                                            oval(CX,(CAP+BASE)//2,W//4,(BASE-CAP)//4,'counter',True),
                                            arc(CX+W//4,(CAP+BASE)//2,W//4,(BASE-CAP)//4,270,90,S,'arc')],
    '#': lambda L,R,W,CX,S,T,sf,fam,adv: [vbar(CX-W//5,CAP+S,BASE-S,S//2,'stem'),
                                            vbar(CX+W//5,CAP+S,BASE-S,S//2,'stem'),
                                            hbar(L+S,R-S,(CAP+BASE)//2-S*2,S//2,'bar'),
                                            hbar(L+S,R-S,(CAP+BASE)//2+S*2,S//2,'bar')],
    '&': lambda L,R,W,CX,S,T,sf,fam,adv: [arc(CX,CAP+W//3,W//3,W//3,0,360,S,'bowl'),
                                            diag(L+S,BASE,R-S,CAP+W//3,S,'stem')],
    # Quotes
    '"': lambda L,R,W,CX,S,T,sf,fam,adv: [vbar(CX-S*2,CAP,CAP+S*3,S//2,'stem'),
                                             vbar(CX+S*2,CAP,CAP+S*3,S//2,'stem')],
    "'": lambda L,R,W,CX,S,T,sf,fam,adv: [vbar(CX,CAP,CAP+S*3,S//2,'stem')],
    # Misc
    '^': lambda L,R,W,CX,S,T,sf,fam,adv: [diag(L+W//4,CAP+W//3,CX,CAP,S,'stem'),
                                             diag(CX,CAP,R-W//4,CAP+W//3,S,'stem')],
    '~': lambda L,R,W,CX,S,T,sf,fam,adv: [arc(CX-W//4,(CAP+BASE)//2,W//5,S*2,0,-180,S,'arc'),
                                             arc(CX+W//4,(CAP+BASE)//2,W//5,S*2,180,0,S,'arc')],
    '<': lambda L,R,W,CX,S,T,sf,fam,adv: [diag(R-S,CAP+W//5,L+S,(CAP+BASE)//2,S,'stem'),
                                             diag(L+S,(CAP+BASE)//2,R-S,BASE-W//5,S,'stem')],
    '>': lambda L,R,W,CX,S,T,sf,fam,adv: [diag(L+S,CAP+W//5,R-S,(CAP+BASE)//2,S,'stem'),
                                             diag(R-S,(CAP+BASE)//2,L+S,BASE-W//5,S,'stem')],
    '%': lambda L,R,W,CX,S,T,sf,fam,adv: [oval(L+W//5,CAP+W//5,W//6,W//6,'bowl'),
                                             oval(R-W//5,BASE-W//5,W//6,W//6,'bowl'),
                                             diag(R-S,CAP,L+S,BASE,S//2,'stem')],
}
