...
ONNX SRCNN deep learning — pikseller arası boşlukları AI ile tahmin eder
"""
...
def _build_model(path):
    ...
    b1=np.zeros(64,dtype=np.float32); b2=np.zeros(32,dtype=np.float32); b3=np.zeros(1,dtype=np.float32)
    nodes=[
    ...
    inits=[numpy_helper.from_array(f1,'f1'),numpy_helper.from_array(f2,'f2'),
    ...
    print(f"[SRCNN] Model: {os.path.getsize(path)//1024}KB")
...
def upscale_image(image_bytes, scale=4.0, denoise_strength=5,
    ...
    if img_np is None: raise ValueError("Invalid image format")
    ...
    # Denoise — sadece gerekirse, rengi koru
    if is_noisy and denoise_strength > 0:
        img_np = cv2.fastNlMeansDenoisingColored(img_np, None,
            h=min(denoise_strength,8), hColor=min(denoise_strength,8),
    ...
    while rem > 1.01:
        step = min(rem, 2.0)
        img_pil = img_pil.resize((int(img_pil.width*step), int(img_pil.height*step)), Image.LANCZOS)
        rem /= step

    # SRCNN AI enhancement — YCbCr uzayında sadece Y kanalı
    # Renk kanallarına (Cb,Cr) dokunulmaz → renk bozulmaz
    ai_used = False
    try:
        ...
        print(f"[SRCNN] skip: {e}")

    # Unsharp mask — hafif, sadece bulanık görüntüde
    if sharpen_amount > 0 and is_blurry:
        img_pil = img_pil.filter(ImageFilter.UnsharpMask(
            radius=1.5, percent=min(sharpen_amount,150), threshold=4))
    elif sharpen_amount > 0:
        img_pil = img_pil.filter(ImageFilter.UnsharpMask(
            radius=0.8, percent=min(sharpen_amount//3,60), threshold=5))
    ...
    return out_bytes, {
        'original_w':int(orig_w),'original_h':int(orig_h),
        'output_w':int(final_w),'output_h':int(final_h),
